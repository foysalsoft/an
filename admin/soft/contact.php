...
<?php $table_heading = "Contact Info";?> 
...
<?php include 'include/body-top.php';?>
 <?php
    ...
    $class='';
        ...
        //  {
        //      $ID = $_GET['delete'];
        //      $sql = "UPDATE $tbl_name SET `IS_DELETED` = 1 ,`DELETED_BY` = '$user_no', `DELETED_ON` = '$CURR_TIME' WHERE CONTACT_NO  = $ID";
    ...
    //   $sql = "INSERT INTO $tbl_name SET `EMAIL`='$EMAIL',`PHONE`='$PHONE',`EN_DESCRIPTION`='$EN_DESCRIPTION',`BN_DESCRIPTION`='$BN_DESCRIPTION',`CREATED_BY` = '$user_no', `CREATED_ON` = '$CURR_TIME'";
    //         $result = mysqli_query($con,$sql);
    //         if($result)
    //             {
               ...
               if(isset($_POST['update']))
                {
                    ...
                    $SQL = "SELECT * FROM $tbl_name WHERE `EMAIL`='$EMAIL' AND `CONTACT_NO`!= '$CONTACT_NO'";
                        $COUNT = mysqli_num_rows(mysqli_query($con,$SQL));
                        if($COUNT < 1): 
                        
                        $sql = "UPDATE $tbl_name SET  `EMAIL`='$EMAIL',`PHONE`='$PHONE',`IS_UPDATED`=1,`UPDATED_BY` = '$user_no', `UPDATED_ON` = '$CURR_TIME' WHERE `CONTACT_NO` = '$CONTACT_NO'";
...
?> 
    <?php
        if(isset($_GET['edit'])):
        $id = $_GET['edit'];
        $sql = "SELECT * FROM $tbl_name WHERE `CONTACT_NO`= '$id' ";
    ...
    ?>
     ...
     <div class="form-group " <?php if($mgs=='')echo "style='display:none;'" ?>>
            ...
            <div>
                <input type="hidden" name="CONTACT_NO" value="<?=$result['CONTACT_NO']?>" />
            </div>
        </div>
         ...
         <div class="form-group ">
            ...
            <div class="col-lg-4">
                <input class=" form-control" id="EMAIL" name="EMAIL" value="<?=$result['EMAIL']?>" type="text"    />
            </div>

            <label for="PHONE" class="control-label col-lg-2">Phone</label>
            <div class="col-lg-4">
                <input class=" form-control" id="PHONE" name="PHONE" value="<?=$result['PHONE']?>"type="text"    />
            </div>
   ...
   <!-- <form class="cmxform form-horizontal " id="signupForm" method="post" enctype="multipart/form-data">
       ...
       <div class="form-group ">
            <label for="EMAIL" class="control-label col-lg-2">Faq Title (English)</label>
            <div class="col-lg-4">
                <input class=" form-control" id="EMAIL" name="EMAIL" value="" type="text"    />
            </div>

            <label for="PHONE" class="control-label col-lg-2">Faq Title (Bangla)</label>
            <div class="col-lg-4">
...
<?php
    ...
    //     $EMAIL =mysqli_real_escape_string($con,trim($_POST['EMAIL']));
    //       if($EMAIL != ""){
    //         $where.=" AND $tbl_name.`EMAIL` LIKE '%$EMAIL%'";
    //       }
    ...
    */
    ...
    /*Sql query for showing data to user*/
     $sql = "SELECT * FROM $tbl_name   WHERE `IS_DELETED` = 0  $where  ORDER BY $tbl_name.`CONTACT_NO` DESC LIMIT $start, $limit";
    $result = mysqli_query($con,$sql);
...
<div style="overflow: auto">
    <table   class="table table-bordered table-hover table-responsive table-condensed table-striped dataTable col-xs-12 col-sm-12 col-md-6 col-lg-6 ">
        ...
        <tr>
            ...
            <td><?=$row['PHONE']?></td>
            <td>
               <center> <a onclick="return confirm('Are you Sure Want to Edit?');" href="<?=$targetpage.'?edit='.$row['CONTACT_NO']?>" class="btn btn-primary"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> </a>
